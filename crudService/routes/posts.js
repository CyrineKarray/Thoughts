...
router.get("/:token", async (req, res) => {
  axios
    .get("http://auth-service:3002/auth/isAuthenticated", {
      headers: {
    ...
    .then(async function (response) {
      ...
      const posts = await Post.find({ author: { $ne: decoded.username } });
...
router.put("/:token", async (req, res) => {
  axios
    ...
    .then(async function (response) {
      ...
      axios.post("http://rate-service:3001/rate/" + req.params.token, rate);
